...
@router.get("", response_model=list[WebhookResponse], summary="List organization webhooks")
async def list_webhooks(
    ...
    if not admin.tenant_id:
        return []
    stmt = select(Webhook).where(Webhook.tenant_id == admin.tenant_id).order_by(Webhook.created_at.desc())
...
async def delete_webhook(
    ...
    stmt = select(Webhook).where(Webhook.id == webhook_id, Webhook.tenant_id == admin.tenant_id)
    result = await db.execute(stmt)
