...
@router.get(
    "/stats",
    summary="Get Reports Center Statistics",
        ...
        data.append(
            {
                ...
                "category": (
                    r.generation_config.get("category", "Standard")
...
async def get_report_related(
    ...
    if report.dataset_id:
        ...
        if ds:
            assets.append({"id": str(ds.id), "name": ds.name, "type": "dataset"})
...
async def create_schedule(
    ...
    schedule = ReportSchedule(
        ...
        email_recipients=(
            ",".join(req.email_recipients) if req.email_recipients else None
        ),
...
async def get_report(
    ...
    if current_user.role not in ["org_admin", "owner"]:
        ...
        if not workspace:
            raise HTTPException(
                status_code=403,
                detail="Workspace context required to access this report.",
            )
        base_conditions.append(Report.workspace_id == workspace.id)
    ...
    return {
        ...
        "data": {
            ...
            "category": (
                report.generation_config.get("category", "Standard")
...
async def simulate_report_workflow(
    tenant_id: uuid.UUID,
    report_id: uuid.UUID,
    user_id: uuid.UUID,
    dataset_id: uuid.UUID,
    ...
    async with AsyncSessionLocal() as db:
        ...
        try:
            ...
            missing_pct = (
                round((missing_cells / total_cells) * 100, 2) if total_cells else 0
            )
...
@router.post(
    ...
    dependencies=[Depends(RequirePermission("REPORT_CREATE"))],
)
...
async def generate_report(
    ...
    if not quota.allowed:
        raise HTTPException(
            status_code=402,
            detail="Reports quota exceeded for your organization's plan.",
        )
    ...
    audit = AuditLog(
        tenant_id=tenant_id,
        user_id=current_user.id,
        action=f"report.{req.report_category}.started",
        resource_type="report",
...
@router.post(
    "/{report_id}/action/{action_type}",
    summary="Perform Action on Report",
    dependencies=[Depends(RequirePermission("REPORT_VIEW"))],
)
        ...
        if action_type in ["delete", "archive"]:
            if current_user.role in ["owner", "org_admin", "organization-admin"]:
                ...
                if "report:delete" not in user_perms:
                    if (
                        "report:delete_own" in user_perms
                        and r.created_by_id == current_user.id
                    ):
                        pass  # Allowed
                    else:
                        raise HTTPException(
...
@router.get(
    ...
    dependencies=[Depends(RequirePermission("REPORT_EXPORT"))],
)
