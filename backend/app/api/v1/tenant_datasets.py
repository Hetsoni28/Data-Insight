...
async def get_dataset_activities(
    ...
    ).order_by(desc(AIUsageLog.created_at)).limit(20)
...
@router.post("/{dataset_id}/analyze", summary="Analyze Dataset")
async def analyze_dataset(
    ...
    audit = AuditLog(
        ...
        resource_type="dataset", resource_id=str(d.id), ip_address="127.0.0.1"
    )
    db.add(audit)
    ...
    ai_log = AIUsageLog(
        tenant_id=tenant_id, task_type="data_profiling",
...
@router.post("/{dataset_id}/ai-excel", summary="Generate AI Excel")
async def generate_ai_excel(
    ...
    ai_log = AIUsageLog(
        tenant_id=tenant_id, task_type="excel_generation",
...
@router.post("/{dataset_id}/dashboard", summary="Create Dashboard")
async def create_dashboard(
    ...
    ai_log = AIUsageLog(
        tenant_id=tenant_id, task_type="dashboard_generation",
