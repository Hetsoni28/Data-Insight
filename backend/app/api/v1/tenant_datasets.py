...
@router.get(
    "/stats",
    summary="Get Dataset Center Statistics",
...
async def get_dataset_stats(
    ...
    try:
        ...
        ai_tasks_stmt = (
            select(AITokenUsage.feature, func.count(AITokenUsage.id))
        ...
        for feature, count in ai_tasks_res:
            if feature == "report_generation":
                ai_reports = count
            elif feature == "excel_generation":
                ai_excel = count
            elif feature == "dashboard_generation":
                dashboards = count
...
async def list_datasets(
    ...
    stmt = (
        select(Dataset, User)
        .outerjoin(User, Dataset.uploaded_by_id == User.id)
...
@router.get(
    "/activities",
    summary="Recent AI Activities & Audit Log",
    dependencies=[Depends(RequirePermission("DATASET_VIEW"))],
)
async def get_dataset_activities(
    ...
    try:
        ...
        return {
            "status": "success",
            "data": {"audit_logs": audit_logs, "ai_activities": ai_logs},
        }
            ...
            mapped_columns.append(
                {
                    ...
                    "sample": (
                        val.get("top_values", [{"value": "N/A"}])[0].get("value")
...
@router.get(
    "/{dataset_id}/charts",
    summary="Get Auto-Generated Charts",
    dependencies=[Depends(RequirePermission("DATASET_VIEW"))],
)
...
async def get_dataset_preview(
    ...
    try:
        ...
        mapped_columns = [
            {"name": col["name"], "type": col["dtype"]} for col in data["columns"]
        ]
...
@router.post(
    "/upload",
    summary="Register Uploaded Dataset",
    dependencies=[Depends(RequirePermission("DATASET_UPLOAD"))],
)
...
@router.delete(
    ...
    dependencies=[Depends(RequirePermission("DATASET_DELETE"))],
)
...
@router.post(
    ...
    dependencies=[Depends(RequirePermission("DATASET_ANALYZE"))],
)
...
async def analyze_dataset(
    ...
    try:
        ...
        audit = AuditLog(
            tenant_id=tenant_id,
            user_id=current_user.id,
            action="dataset.analyze.started",
            resource_type="dataset",
            resource_id=str(d.id),
            ip_address=request.client.host if request.client else "127.0.0.1",
        )
        ...
        ai_log = AITokenUsage(
            ...
            cost_usd=float(tokens + 50) * 0.00001,
        )
        db.add(ai_log)
...
@router.post(
    "/{dataset_id}/ai-excel",
    summary="Generate AI Excel",
    dependencies=[Depends(RequirePermission("DATASET_AI_EXCEL"))],
)
...
async def generate_ai_excel(
    ...
    if not can_use_feature(tenant, BillingFeature.AI_EXCEL):
        raise HTTPException(
            status_code=403,
            detail="AI Excel generation is not included in your organization's plan.",
        )
    ...
    audit = AuditLog(
        tenant_id=tenant_id,
        user_id=current_user.id,
        action="dataset.ai-excel.started",
        resource_type="dataset",
        resource_id=str(d.id),
        ip_address=request.client.host if request.client else "127.0.0.1",
    )
...
@router.post(
    ...
    dependencies=[Depends(RequirePermission("DATASET_CREATE_DASHBOARD"))],
)
@limiter.limit("5/minute")
async def create_dashboard(
    ...
    try:
        ...
        audit = AuditLog(
            tenant_id=tenant_id,
            user_id=current_user.id,
            action="dataset.dashboard.started",
            resource_type="dataset",
            resource_id=str(d.id),
            ip_address=request.client.host if request.client else "127.0.0.1",
        )
...
@router.post(
    "/{dataset_id}/nl-chart",
    summary="Generate Natural Language Chart",
    dependencies=[Depends(RequirePermission("DATASET_VIEW"))],
)
@limiter.limit("5/minute")
...
async def get_dataset_alerts(
    ...
    stmt = select(DatasetAlert).where(
        DatasetAlert.dataset_id == dataset_id,
        DatasetAlert.tenant_id == current_user.tenant_id,
    )
    res = await db.execute(stmt)
...
@router.delete(
    "/{dataset_id}/alerts/{alert_id}",
    summary="Delete Dataset Alert",
    dependencies=[Depends(RequirePermission("DATASET_EXPORT"))],
)
...
@router.post(
    "/{dataset_id}/cleaning/suggestions",
    summary="Get AI Cleaning Suggestions",
    dependencies=[Depends(RequirePermission("DATASET_EXPORT"))],
)
@limiter.limit("5/minute")
