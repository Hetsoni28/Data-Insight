...
async def get_full_profile(
    ...
    if tenant_id:
        users_managed = (
            await db.scalar(select(func.count()).where(User.tenant_id == tenant_id))
            or 0
        )
        reports_generated = (
            await db.scalar(select(func.count()).where(Report.tenant_id == tenant_id))
            or 0
        )
        datasets_uploaded = (
            await db.scalar(select(func.count()).where(Dataset.tenant_id == tenant_id))
            or 0
        )
        ai_requests = (
            await db.scalar(
    ...
    stats = ProfileStatsResponse(
        ...
        storage_used_mb=(
            int(storage_used_bytes / (1024 * 1024)) if storage_used_bytes else 0
        ),
