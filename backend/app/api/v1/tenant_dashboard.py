...
# ─── SECURITY ──────────────────────────────────────────────────────────────────
@router.get("/security", summary="Security & Access", dependencies=[Depends(RequireRole(["org_admin"]))])
async def get_security_overview(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="An error occurred while fetching security data.")
