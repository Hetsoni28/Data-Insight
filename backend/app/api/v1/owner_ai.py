...
async def get_ai_overview(
    ...
    avg_health = (
        sum(p.health_score for p in providers) / total_providers
        if total_providers
        else 100.0
    )
    ...
    prev_success = 100.0 - (
        (prev_errors / prev_requests * 100) if prev_requests > 0 else 0.0
    )
    ...
    return {
        ...
        "average_cost_per_request": (
            (total_cost / total_requests) if total_requests else 0
        ),
...
async def get_providers(
    ...
    for p, reqs, tokens, cost, avg_lat, errors in result.all():
        ...
        success_rate = (
            round(100.0 - (err_count / req_count * 100), 1) if req_count > 0 else 100.0
        )
...
async def get_routing_rules(
    ...
    for r in rules:
        p_model = (
            await db.get(AIModel, r.primary_model_id) if r.primary_model_id else None
        )
        f_model = (
            await db.get(AIModel, r.fallback_model_id) if r.fallback_model_id else None
        )
...
async def get_trends(
    ...
    for t in res["timeseries"]:
        data.append({"name": t["date"], "requests": t["requests"], "cost": t["cost"]})
...
async def chat_with_command_center(
    ...
    # Save User Message
    user_msg = ChatMessage(session_id=session_uuid, role="user", content=message)
    db.add(user_msg)
    await db.commit()
