...
async def get_team_stats(
    ...
    online_stmt = (
        ...
        .where(User.tenant_id == tenant_id, UserSession.last_active_at >= ten_mins_ago)
    )
...
async def get_team_departments(
    ...
    stmt = (
        ...
        .where(TenantDepartment.tenant_id == tenant_id)
        .offset(skip)
        .limit(limit)
    )
    result = await db.execute(stmt)
...
async def invite_team_member(
    ...
    if not quota.allowed:
        raise HTTPException(
            status_code=402,
            detail="User seat limit exceeded for your organization's plan.",
        )
