...
async def get_overview(
    ...
    today_req_result = await db.execute(
        select(func.count(ApiRequestLog.id))
        .where(ApiRequestLog.created_at >= today)
    )
    ...
    error_req_result = await db.execute(
        select(func.count(ApiRequestLog.id))
        .where(and_(ApiRequestLog.created_at >= thirty_days_ago, ApiRequestLog.status_code >= 400))
    )
    error_requests = error_req_result.scalar_one_or_none() or 0
    
    # Average Latency
    avg_latency_result = await db.execute(
        select(func.avg(ApiRequestLog.latency_ms))
        .where(ApiRequestLog.created_at >= thirty_days_ago)
    )
    ...
    apps_result = await db.execute(
        select(func.count(OAuthClient.id))
    )
    ...
    return {
        ...
        "api_availability": 99.99 if total_requests == 0 else round(100 - ((error_requests / total_requests) * 100), 2)
    }
...
async def get_security_overview(
    ...
    rate_limit_result = await db.execute(
        select(func.count(ApiRequestLog.id))
        .where(and_(ApiRequestLog.created_at >= thirty_days_ago, ApiRequestLog.status_code == 429))
    )
    ...
    auth_failure_result = await db.execute(
        select(func.count(ApiRequestLog.id))
        .where(and_(ApiRequestLog.created_at >= thirty_days_ago, ApiRequestLog.status_code.in_([401, 403])))
    )
