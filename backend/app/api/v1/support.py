...
from app.schemas.support import (
    ...
    PlatformIncidentCreate, PlatformIncidentUpdate, PlatformIncidentResponse
)
...
async def get_support_dashboard(
    ...
    pending_tickets_query = await db.execute(select(func.count()).where(SupportTicket.status == TicketStatus.IN_PROGRESS))
    pending_tickets = pending_tickets_query.scalar() or 0
    
    critical_query = await db.execute(select(func.count()).where(SupportTicket.priority == TicketPriority.CRITICAL, SupportTicket.status != TicketStatus.CLOSED))
...
async def create_ticket(
    ...
    await AuditService.log(
        ...
        "support.ticket.create", f"Created support ticket: {ticket.id}"
    )
...
async def update_ticket(
    ...
    if current_user.role != UserRole.owner and ticket.tenant_id != current_user.tenant_id:
        raise HTTPException(status_code=403, detail="Not authorized to update this ticket")

    update_data = ticket_in.dict(exclude_unset=True)
    
    if "status" in update_data and update_data["status"] == TicketStatus.RESOLVED and ticket.status != TicketStatus.RESOLVED:
    ...
    await AuditService.log(
        db, current_user.id, current_user.tenant_id,
        "support.ticket.update", f"Updated ticket {ticket.id}"
    )
...
async def delete_ticket(
    ...
    await AuditService.log(
        db, current_user.id, current_user.tenant_id,
        "support.ticket.delete", f"Deleted ticket {ticket_id}"
    )
...
async def create_incident(
    ...
    await AuditService.log(
        db, current_user.id, current_user.tenant_id,
        "support.incident.create", f"Reported platform incident: {incident.title}"
    )
    return incident
...
async def send_broadcast(
    ...
    await AuditService.log(
        db, current_user.id, current_user.tenant_id,
        "support.broadcast.send", f"Sent platform broadcast: {payload.get('message', 'No message')[:50]}"
    )
