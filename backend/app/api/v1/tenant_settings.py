...
_create_config_endpoints(router, "data-connections", "data_connections_config", "Data Connections Config")
_create_config_endpoints(router, "integrations", "integrations_config", "Integrations Config")
_create_config_endpoints(router, "notifications", "notifications_config", "Notifications Config")
...
async def get_tenant_audit_logs(
    ...
    stmt = select(AuditLog, User.email).outerjoin(User, AuditLog.user_id == User.id).where(AuditLog.tenant_id == tenant_id).order_by(AuditLog.created_at.desc()).limit(50)
    res = await db.execute(stmt)
...
async def connect_integration(
    ...
    return {"status": "success", "message": f"{req.provider.capitalize()} connected successfully."}
...
async def test_notification(
    ...
    return {"status": "success", "message": f"Test {req.type} notification sent to {current_user.email}."}
