...
async def _process_ai_report(tenant_id: str, report_id: str, dataset_id: str, report_category: str):
    """
    ...
    async with AsyncSessionLocal() as db:
        ...
        r_stmt = select(Report).where(Report.id == uuid.UUID(report_id), Report.tenant_id == uuid.UUID(tenant_id))
        report = (await db.execute(r_stmt)).scalars().first()
        if not report:
            return
        
        d_stmt = select(Dataset).where(Dataset.id == uuid.UUID(dataset_id), Dataset.tenant_id == uuid.UUID(tenant_id))
            ...
            if report_category == "executive":
                ...
                schema = {
                    ...
                    "properties": {
                        ...
                        "managementActionPlan": {"type": "ARRAY", "items": {"type": "STRING"}},
                        "executiveConclusion": {"type": "STRING"}
                    },
                    "required": ["companyOverview", "datasetSummary", "businessHighlights", "executiveKPIs", "revenueOverview", "profitAnalysis", "growthAnalysis", "topInsights", "businessOpportunities", "potentialRisks", "keyRecommendations", "managementActionPlan", "executiveConclusion"]
                }
                ...
                schema = {
                    ...
                    "properties": {
                        ...
                        "businessRecommendations": {"type": "ARRAY", "items": {"type": "STRING"}},
                        "forecastOpportunities": {"type": "ARRAY", "items": {"type": "STRING"}},
                        "confidenceScores": {
                            "type": "OBJECT",
                            "properties": {
                        ...
                        "charts": {
                            ...
                            "items": {
                                ...
                                "properties": {
                                    "chartId": {"type": "STRING"},
                                    "title": {"type": "STRING"},
                                    "chartType": {"type": "STRING"}, # "bar", "line", "pie", "area"
