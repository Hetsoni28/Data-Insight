...
async def seed_ai_ops():
    async with AsyncSessionLocal() as db:
        ...
        models = [
            ...
            # Anthropic
            AIModel(
                provider_id=provider_map["Anthropic"].id,
                name="Gemini 3.5 Flash",
                model_id_string="claude-3-5-sonnet-20240620",
                type=ModelType.CHAT,
            ...
            AIModel(
                provider_id=provider_map["Anthropic"].id,
                name="Gemini 3.5 Flash",
                model_id_string="claude-3-opus-20240229",
            ...
            # Google
            AIModel(
                provider_id=provider_map["Google Vertex AI"].id,
                name="Gemini 1.5 Pro",
            ...
            # Groq
            AIModel(
                provider_id=provider_map["Groq"].id,
                name="Llama 3 70B",
                model_id_string="llama3-70b-8192",
                type=ModelType.CHAT,
                context_window=8192,
        ...
        prompts = [
            AIPromptTemplate(
                ...
                user_prompt_template="Generate a report for {dataset_name} covering {metrics}.",
                variables=["dataset_name", "metrics"],
            ),
            AIPromptTemplate(
                name="Data Cleaning Agent",
                version=1,
                system_prompt="You are a data cleaner...",
                user_prompt_template="Clean the following JSON array: {data}",
                variables=["data"],
            ),
        ]
        db.add_all(prompts)
        ...
        for _ in range(1000):
            ...
            latency = (
                int(random.gauss(800, 200))
                if m.type == ModelType.CHAT
                else int(random.gauss(300, 50))
            )

            # Success/Failure (98% success)
            status_code = (
                200 if random.random() > 0.02 else random.choice([429, 500, 503])
            )
