...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.String(length=50),
            nullable=False,
            server_default="Annual Advance",
        ),
        sa.Column(
            "support_tier",
            sa.String(length=50),
            nullable=False,
            server_default="24/7 Dedicated Engineering",
        ),
        sa.Column(
            "sla_guarantee",
            sa.String(length=50),
            nullable=False,
            server_default="99.99% Uptime SLA",
        ),
        sa.Column(
            "deployment_model",
            sa.String(length=100),
            nullable=False,
            server_default="Dedicated Single-Tenant VPC",
        ),
    ...
    op.create_index(
        op.f("ix_rental_contracts_tenant_id"),
        "rental_contracts",
        ["tenant_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_rental_contracts_contract_number"),
        "rental_contracts",
        ["contract_number"],
        unique=True,
    )
    ...
    op.create_index(
        op.f("ix_resource_requests_tenant_id"),
        "resource_requests",
        ["tenant_id"],
        unique=False,
    )
