...
from sqlalchemy import String, DateTime, Boolean, ForeignKey, Integer, JSON
...
class FeatureFlag(Base):
    ...
    tags: Mapped[dict | None] = mapped_column(JSON, nullable=True)  # Categorization like 'ai', 'billing'
    
    # Relationships
    rollouts = relationship("FeatureRollout", back_populates="flag", cascade="all, delete-orphan")
    experiments = relationship("FeatureExperiment", back_populates="flag", cascade="all, delete-orphan")
...
class FeatureRollout(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    flag_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("feature_flags.id", ondelete="CASCADE"))
    
    rollout_percentage: Mapped[int] = mapped_column(Integer, default=0) # 0 to 100
    target_roles: Mapped[dict | None] = mapped_column(JSON, nullable=True) # ['owner', 'admin']
    target_organizations: Mapped[dict | None] = mapped_column(JSON, nullable=True) # ['org1_id', 'org2_id']
