...
class AIProvider(Base):
    ...
    api_key_secret = Column(String, nullable=True) # Will store standard string for MVP, can upgrade to Fernet later
...
class AIModel(Base):
    ...
    routing_rules_primary = relationship("AIRoutingRule", foreign_keys="AIRoutingRule.primary_model_id", back_populates="primary_model")
    routing_rules_fallback = relationship("AIRoutingRule", foreign_keys="AIRoutingRule.fallback_model_id", back_populates="fallback_model")
...
class AIRoutingRule(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, index=True)
    task_type = Column(String, nullable=False, unique=True) # e.g., 'default_chat', 'excel_generation', 'vision_tasks'
    primary_model_id = Column(UUID(as_uuid=True), ForeignKey("ai_models.id", ondelete="SET NULL"), nullable=True)
    ...
    primary_model = relationship("AIModel", foreign_keys=[primary_model_id], back_populates="routing_rules_primary")
    fallback_model = relationship("AIModel", foreign_keys=[fallback_model_id], back_populates="routing_rules_fallback")
