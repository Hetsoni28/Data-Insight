...
class ApiRequestLog(Base):
    ...
    ip_address: Mapped[str | None] = mapped_column(String(45), nullable=True) # Supporting IPv6 length
...
class ApiRateLimit(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    workspace_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("workspaces.id", ondelete="CASCADE"), nullable=True, unique=True) # Null = Global
...
class ApiIntegration(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    provider: Mapped[str] = mapped_column(String(50), nullable=False, unique=True) # 'stripe', 'openai', 'supabase'
    
    is_active: Mapped[bool] = mapped_column(Boolean, default=False)
    health_status: Mapped[str] = mapped_column(String(20), default="unknown") # 'healthy', 'degraded', 'offline'
