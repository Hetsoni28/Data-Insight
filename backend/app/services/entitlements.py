...
PLAN_CATALOG = {
    ...
    "professional": PlanLimits(
        ...
        reports=100,
        dashboards=50,
        features=[BillingFeature.AI_EXCEL, BillingFeature.FORECASTING, BillingFeature.API_ACCESS]
    ),
...
async def get_usage(tenant: Tenant, db: AsyncSession) -> UsageMap:
    ...
    reports_count = await db.scalar(
        ...
        .where(Report.tenant_id == tenant.id, Report.is_deleted == False)
    )
    
    # Active Dashboards
    dashboards_count = await db.scalar(
        select(func.count(Dashboard.id))
        .where(Dashboard.tenant_id == tenant.id, Dashboard.is_deleted == False)
    )
...
def check_quota(tenant: Tenant, usage: UsageMap, resource: BillingResource, buffer: int = 1) -> QuotaResult:
    """
    ...
    if resource == BillingResource.USERS:
        ...
        limit = plan_limit + (getattr(tenant, "seats_purchased", 0) or 0) if plan_limit is not None else None
    ...
    if limit is None:
        return QuotaResult(allowed=True, used=current_used, limit=limit, remaining=None, warning_state=warning_state)
