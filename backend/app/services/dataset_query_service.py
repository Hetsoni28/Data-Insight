...
class DatasetQueryService:
    @staticmethod
    async def execute_query(dataset_id: uuid.UUID, tenant_id: uuid.UUID, workspace_id: uuid.UUID | None, query_payload: Dict[str, Any], dataset_metadata: Dict[str, Any] = None, storage_path: str = None) -> List[Dict[str, Any]]:
        """
        ...
        if limit > 10000:
            raise ValueError("Query limit cannot exceed 10,000 rows for performance reasons")

        # 2. Schema Validation (Strong Logic)
        if dataset_metadata and "schema" in dataset_metadata:
            valid_columns = {col["name"]: col["type"] for col in dataset_metadata.get("schema", [])}
            
            # Validate Dimensions
            for dim in dimensions:
                if dim not in valid_columns:
                    raise ValueError(f"Invalid dimension requested: '{dim}'. Column does not exist in dataset schema.")
            ...
            for metric in metrics:
                ...
                if field not in valid_columns:
                    raise ValueError(f"Invalid metric requested: '{field}'. Column does not exist in dataset schema.")
                if agg not in allowed_aggregations:
                    raise ValueError(f"Invalid aggregation: '{agg}'. Allowed aggregations are: {', '.join(allowed_aggregations)}")
                    
            # Validate Filters
            for f in filters:
                field = f.get("field")
                if field not in valid_columns:
                    raise ValueError(f"Invalid filter field: '{field}'. Column does not exist in dataset schema.")
        ...
        for m in metrics:
            ...
            if agg == "COUNT_DISTINCT":
                select_cols.append(f'COUNT(DISTINCT "{field}") as "count_distinct_{field}"')
            else:
