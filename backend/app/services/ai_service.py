...
Do not include markdown code block syntax (like ```json), just the raw JSON object."""
            ...
            try:
                ...
                if not self._openai:
                    raise AIServiceException(
                        f"Blueprint generation failed (Gemini): {str(e)}"
                    )
        ...
        if self._openai:
            ...
            try:
                ...
                raise AIServiceException(
                    f"Blueprint generation failed (OpenAI): {str(e)}"
                )
...
Write in authoritative, professional business English. Focus on WHY trends happened and WHAT the business should do next."""
        ...
        if self._gemini:
            try:
                ...
                if not self._openai:
                    raise AIServiceException(
                        f"Summary generation failed (Gemini): {str(e)}"
                    )
        ...
        if self._openai:
            ...
            try:
                ...
                raise AIServiceException(
                    f"Summary generation failed (OpenAI): {str(e)}"
                )
