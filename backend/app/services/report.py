...
from app.core.exceptions import ResourceNotFoundException, ForbiddenException, TenantQuotaExceededException
...
class ReportService:
    ...
    async def generate_report(
        ...
        if dataset.status != DatasetStatus.ready:
            raise ForbiddenException("Dataset must finish profiling before generating a report.")
    ...
    async def list_reports(self, workspace_id: uuid.UUID, actor: User) -> list[Report]:
        if not actor.tenant_id:
            return []
        return await self.report_repo.get_workspace_reports(actor.tenant_id, workspace_id)
    ...
    async def approve_report(self, report_id: uuid.UUID, actor: User, notes: str | None = None) -> Report:
        ...
        await self.report_repo.save(report)
        await self.audit_repo.log("report.approve", tenant_id=actor.tenant_id, user_id=actor.id,
    ...
    async def delete_report(self, report_id: uuid.UUID, actor: User) -> None:
        ...
        await self.audit_repo.log("report.delete", tenant_id=actor.tenant_id, user_id=actor.id,
                                  resource_type="report", resource_id=str(report.id))
