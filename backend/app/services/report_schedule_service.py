...
class ReportScheduleService:
    ...
    async def create_schedule(self, dataset_id: uuid.UUID, cron_expression: str, name: str, actor: User) -> ReportSchedule:
    ...
    async def delete_schedule(self, schedule_id: uuid.UUID, actor: User) -> None:
        stmt = select(ReportSchedule).where(ReportSchedule.id == schedule_id, ReportSchedule.tenant_id == actor.tenant_id)
        result = await self.session.execute(stmt)
