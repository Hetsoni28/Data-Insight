...
class ManagerAnalyticsService:
    ...
    async def get_kpis(self, dataset_id: uuid.UUID, actor: User) -> Dict[str, Any]:
        ...
        # Calculate KPI for up to 4 numeric columns
        for i, col in enumerate(numeric_cols[:4]):
            ...
            try:
                ...
                total = result.get("data", [{}])[0].get("total", 0) or 0
                
                # We won't have previous value without a date col, so mock previous temporarily
                ...
                kpis.append({
                    ...
                    "is_currency": "revenue" in col.lower() or "price" in col.lower() or "cost" in col.lower()
                })
    ...
    async def get_trends(self, dataset_id: uuid.UUID, actor: User) -> Dict[str, Any]:
        ...
        if date_cols and numeric_cols:
            ...
            try:
                ...
                for row in data:
                    if row.get("month"):
                        # row['month'] might be a string depending on DuckDB returning it
                        trend_items.append({
    ...
    async def get_performance(self, dataset_id: uuid.UUID, actor: User) -> Dict[str, Any]:
        ...
        if categorical_cols and numeric_cols:
            ...
            # Analyze top 2 categorical columns
            for i, cat_col in enumerate(categorical_cols[:2]):
                ...
                try:
                    ...
                    total_sum = sum((r.get("total", 0) or 0) for r in data)
    ...
    async def get_anomalies(self, dataset_id: uuid.UUID, actor: User) -> Dict[str, Any]:
        ...
        if numeric_cols:
            num_col = numeric_cols[0]
            # Simple static anomaly logic based on overall mean/stddev from profile
            try:
                mean = ds.profile["columns"][num_col].get("mean", 0)
                std = ds.profile["columns"][num_col].get("std", 1)
                
                # Fetch max value to see if it's anomalous
                sql = f'''
                    SELECT MAX(TRY_CAST("{num_col}" AS DOUBLE)) as max_val 
                    FROM dataset
                '''
                result = await self.dataset_service.execute_query(dataset_id, sql, actor)
                max_val = result.get("data", [{}])[0].get("max_val", 0) or 0
