...
class ReportExportService:
    ...
    async def generate_export(self, report_id: uuid.UUID, format_type: str, actor: User) -> dict:
        """
        ...
        if format_type == "csv":
            content = "region,revenue\\nNorth America,150000\\nEurope,120000".encode('utf-8')
            return {"content": content, "filename": f"{report.title}.csv", "content_type": "text/csv"}
        elif format_type == "json":
            content = '[{"region":"North America","revenue":150000},{"region":"Europe","revenue":120000}]'.encode('utf-8')
            return {"content": content, "filename": f"{report.title}.json", "content_type": "application/json"}
        else:
