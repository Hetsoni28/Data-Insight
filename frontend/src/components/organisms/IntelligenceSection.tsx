              ...
              {FEATURES.map((item) => {
                ...
                return (
                  <motion.div key={item.id} variants={fadeUp}>
                    <AccordionItem value={item.id} className="rounded-2xl border px-5 bg-white data-[state=open]:shadow-md data-[state=open]:border-[#10B981]/30 transition-all">
                      <AccordionTrigger className="hover:no-underline py-4 gap-3">
                        <div className="flex items-center gap-3 text-left">
                          <div className={`h-10 w-10 rounded-xl flex items-center justify-center shrink-0 transition-colors ${activeFeature === item.id ? 'bg-[#10B981] text-white shadow-md' : 'bg-slate-100 text-slate-500'}`}>
                            <Icon className="h-5 w-5" />
                          </div>
                          <span className={`font-bold text-sm transition-colors ${activeFeature === item.id ? 'text-[#10B981]' : 'text-slate-700'}`}>{item.title}</span>
                        </div>
                      </AccordionTrigger>
                      <AccordionContent className="pb-5 space-y-4">
                        ...
                        <div className="grid sm:grid-cols-2 gap-2">
                          {item.bullets.map(b => (
                            <div key={b} className="flex items-start gap-2 text-xs text-slate-600 font-medium">
          ...
          >
            <AnimatePresence mode="wait">
              ...
              {activeFeature === "integration" && (
                <motion.div key="integration" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.3 }} className="absolute inset-0 rounded-2xl bg-white p-6 shadow-xl border border-slate-200 flex flex-col">
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-slate-900 font-bold text-sm">Live Connections</div>
                        <div className="text-slate-500 text-[10px]">Secure Data Tunnels</div>
                      </div>
                    </div>
                  ...
                  <div className="space-y-3 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                    {DB_CONNECTORS.map((db, i) => (
                      <div key={db.name} className={`flex items-center justify-between p-3 rounded-xl border ${db.active ? 'bg-slate-50 border-slate-200 shadow-sm' : 'bg-white border-slate-100'} transition-colors hover:border-[#10B981]/30 cursor-pointer`}>
                        <div className="flex items-center gap-3">
                          <div className={`h-8 w-8 rounded-lg flex items-center justify-center ${db.active ? 'bg-white shadow-sm border' : 'bg-slate-50'}`}>
                            <Database className={`h-4 w-4 ${db.active ? 'text-[#10B981]' : 'text-slate-400'}`} />
                          </div>
                          <div>
                            <div className={`text-xs font-bold ${db.active ? 'text-slate-800' : 'text-slate-500'}`}>{db.name}</div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <Button size="sm" variant={db.active ? "outline" : "ghost"} className={`h-7 px-3 text-[10px] ${db.active ? 'bg-white text-slate-700 hover:text-[#10B981] hover:bg-emerald-50' : 'text-slate-400'}`}>
              ...
              {activeFeature === "copilot" && (
                <motion.div key="copilot" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.3 }} className="absolute inset-0 rounded-2xl bg-white shadow-xl border border-slate-200 flex flex-col overflow-hidden">
                  <div className="bg-white border-b px-4 py-3 flex items-center gap-2 shadow-sm relative z-10">
                    <TerminalSquare className="h-5 w-5 text-[#10B981]" />
                    <span className="font-bold text-sm text-slate-800">Natural Language to SQL</span>
                  </div>
                  <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50">
                    {messages.map((msg, i) => (
                      <motion.div key={i} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className={`flex gap-3 max-w-[90%] ${msg.role === 'user' ? 'ml-auto flex-row-reverse' : ''}`}>
                        <div className={`h-8 w-8 rounded-xl flex items-center justify-center shrink-0 shadow-sm border ${msg.role === 'user' ? 'bg-[#10B981] border-[#059669]' : 'bg-white border-slate-200'}`}>
                          {msg.role === 'user' ? <span className="text-[10px] font-bold text-white">YOU</span> : <Sparkles className="h-4 w-4 text-[#10B981]" />}
                        </div>
                        <div className={`rounded-2xl p-3 text-xs leading-relaxed shadow-sm border ${msg.role === 'user' ? 'bg-[#10B981] text-white rounded-tr-none border-[#059669]' : 'bg-white text-slate-800 rounded-tl-none border-slate-200'}`}>
                          {msg.text && <p className={msg.role === 'user' ? 'font-medium' : ''}>{msg.text}</p>}
                          {msg.sql && (
                            <div className="mt-1.5 bg-slate-50 text-slate-700 border border-slate-100 font-mono p-3 rounded-lg text-[10px] overflow-x-auto whitespace-pre">
                  ...
                  <form onSubmit={handleChat} className="p-3 bg-white border-t flex gap-2">
                    <Input value={chatInput} onChange={e => setChatInput(e.target.value)} placeholder="Type a query (e.g. show revenue by region)..." className="text-xs bg-slate-50 border-slate-200 focus-visible:ring-[#10B981]" />
              ...
              {activeFeature === "automl" && (
                <motion.div key="automl" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.3 }} className="absolute inset-0 rounded-2xl bg-white p-6 shadow-xl border border-slate-200 flex flex-col">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-slate-900 font-bold text-sm">Model Training History</div>
              ...
              {activeFeature === "alerts" && (
                <motion.div key="alerts" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.3 }} className="absolute inset-0 rounded-2xl bg-slate-50 p-6 shadow-xl border border-slate-200 flex flex-col">
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-slate-900 font-bold text-sm">Automated Alerts</div>
                        <div className="text-slate-400 text-[10px]">Real-time KPI Monitoring</div>
                      </div>
                    </div>
                    <Button size="sm" variant="outline" className="h-7 text-[10px] bg-white">Configure Rules</Button>
                  </div>
                  <div className="space-y-3 flex-1 overflow-y-auto pr-1">
                    <AnimatePresence>
                      {alerts.map((alert, i) => (
                        <motion.div key={alert.title} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm flex items-start gap-4 cursor-pointer hover:border-slate-300 transition-colors">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-2 mt-1.5">
                              <Badge variant="secondary" className="text-[9px] font-normal px-1.5 py-0 bg-slate-100 text-slate-600">{alert.metric}</Badge>
