...
const TypingIndicator = () => (
  <div className="flex items-center space-x-1.5 p-2 px-4 bg-white border border-slate-100 shadow-sm rounded-2xl rounded-tl-sm h-10 w-fit">
    <motion.div
...
export function CopilotChat({ datasetId }: CopilotChatProps) {
  ...
  if (!datasetId) {
    return (
      <div className="flex flex-col h-full bg-white dark:bg-slate-950 rounded-2xl border shadow-sm overflow-hidden">
        <StateLayout
  ...
  return (
    <div className="flex flex-col h-full bg-white/60 backdrop-blur-2xl rounded-2xl border border-slate-200/60 shadow-xl overflow-hidden">
      {/* Premium Header */}
      <div className="bg-white/80 backdrop-blur-md border-b border-slate-100 px-6 py-4 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-slate-900 text-base">Data Insight Copilot</h3>
      ...
      <div className="p-4 bg-white/80 backdrop-blur-xl border-t border-slate-100 shrink-0">
        <form onSubmit={handleSend} className="max-w-4xl mx-auto relative group">
          <input
            ...
            disabled={isTyping}
            className="w-full pl-6 pr-14 py-4 rounded-2xl border-slate-200 border bg-white shadow-sm hover:border-emerald-300 focus:outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all duration-300 ease-out text-[15px] disabled:opacity-50"
          />
          <Button 
            type="submit" 
