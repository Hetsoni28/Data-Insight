...
export function SubscriptionsDataGrid() {
  ...
  const getStatusBadge = (status: string) => {
    switch(status) {
      ...
      case 'canceled':
        return <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-slate-100 text-slate-700 border border-slate-200 text-xs font-semibold"><XCircle className="h-3.5 w-3.5" /> Canceled</span>
      default:
        return <span className="px-2.5 py-1 rounded-md bg-slate-100 text-slate-700 border border-slate-200 text-xs font-semibold">{status}</span>
    }
  }
  ...
  if (loading) {
    return (
      <div className="w-full h-96 flex flex-col justify-center items-center bg-white rounded-lg border border-slate-200">
          ...
          <tbody className="divide-y divide-slate-100">
            {filteredSubs.map((sub) => (
              <tr key={sub.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-6 py-4">
                  <div className="font-mono text-slate-500 text-xs">{sub.id}</div>
                </td>
                <td className="px-6 py-4">
                  <div className="flex flex-col">
                    {getPlanBadge(sub.plan)}
                    <span className="text-slate-500 text-xs mt-0.5">${sub.amount.toFixed(2)}</span>
                  </div>
                </td>
                <td className="px-6 py-4">
                  <div className="flex flex-col">
                    <span className="capitalize font-medium text-slate-700">{sub.billing_cycle}</span>
