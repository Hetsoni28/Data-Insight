...
export function ManagerReportTable({
  ...
  return (
    <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden flex flex-col">
            ...
            {isLoading ? (
              ...
              <tr>
                <td colSpan={7} className="px-6 py-12 text-center text-slate-500">
                  ...
                  <p className="mt-1">Try adjusting your filters or generate a new report.</p>
                </td>
              </tr>
            ) : (
              reports.map((report) => (
                <tr key={report.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                  ...
                  <td className="px-6 py-4 font-medium text-slate-900 dark:text-white flex items-center gap-3">
                    ...
                    <span className="truncate max-w-[200px]" title={report.title}>{report.title}</span>
                  </td>
                  <td className="px-6 py-4 text-slate-500 capitalize">{report.report_type}</td>
                  <td className="px-6 py-4">{getStatusBadge(report.status)}</td>
                  <td className="px-6 py-4 text-slate-500">{formatBytes((report as any).output_size_bytes)}</td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button onClick={() => onAction('delete', report.id)} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-500/10 rounded" title="Delete">
      ...
      {!isLoading && reports.length > 0 && totalPages > 0 && (
        <PaginationControls
