...
import { 
  Building2, Users, Search, Filter, Shield, Settings2, Power, Activity, HardDrive, Check
} from "lucide-react"
...
const ORG_COLUMNS = [
  ...
  { id: "plan_mrr",     label: "Plan & MRR" },
  { id: "health",       label: "Health" },
  { id: "usage",        label: "Usage & Storage" },
  { id: "security",     label: "Security" },
] as const

type OrgColId = typeof ORG_COLUMNS[number]["id"]
...
export function OrganizationDataGrid() {
  ...
  const [visibleColumns, setVisibleColumns] = useState<Set<OrgColId>>(
    new Set(ORG_COLUMNS.map(c => c.id))
  )

  const toggleColumn = (id: OrgColId) => {
    setVisibleColumns(prev => {
      const next = new Set(prev)
      if (next.has(id)) {
        if (next.size > 1) next.delete(id)
        else toast.info("At least one column must remain visible.")
      } else {
        next.add(id)
      }
      return next
  ...
  const col = (id: OrgColId) => visibleColumns.has(id)
    ...
    onSuccess: (_data, variables) => {
      ...
      setTenants(prev =>
        (Array.isArray(prev) ? prev : []).map(t =>
          t.id === variables.tenantId ? { ...t, is_active: variables.isActive } : t
        )
      )
  ...
  const filteredTenants = useMemo(() => {
    ...
    return list.filter(t => {
      ...
      const matchesIndustry =
        industryFilter === "All" || (t?.industry || "None") === industryFilter
      ...
      <div className="bg-white dark:bg-white/5 rounded-2xl border border-slate-200/60 dark:border-white/10 shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="p-4 md:p-5 border-b border-slate-200/60 dark:border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50/50 dark:bg-white/5">
          ...
          <div className="flex items-center gap-2">
            <Button
              onClick={() => { setIsFilterOpen(v => !v); setIsColumnPickerOpen(false) }}
            ...
            <Button
              onClick={() => { setIsColumnPickerOpen(v => !v); setIsFilterOpen(false) }}
              variant={isColumnPickerOpen ? "default" : "outline"}
              className={`h-10 shadow-sm rounded-md hidden md:flex border-slate-200/60 dark:border-white/10 ${!isColumnPickerOpen ? "bg-white dark:bg-white/5 text-slate-700 dark:text-slate-300" : ""}`}
            >
              <Settings2 className="h-4 w-4 mr-2" />
              Columns
              {visibleColumns.size < ORG_COLUMNS.length && (
                <span className="ml-1.5 px-1.5 py-0.5 rounded-full bg-emerald-500 text-white text-[10px] font-bold leading-none">
                  {visibleColumns.size}/{ORG_COLUMNS.length}
                </span>
              )}
            </Button>
        ...
        {/* ── Column Picker Panel ── */}
        {isColumnPickerOpen && (
          <div className="p-4 border-b border-slate-200/60 dark:border-white/10 bg-slate-50/50 dark:bg-white/[0.02]">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Toggle Columns</span>
              <button
                onClick={() => setVisibleColumns(new Set(ORG_COLUMNS.map(c => c.id)))}
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {/* Actions is always locked visible */}
              <span
                title="Actions column is always visible"
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-medium bg-slate-100 border-slate-200 text-slate-400 dark:bg-white/5 dark:border-white/10 cursor-not-allowed select-none"
              >
                <Check className="w-3 h-3" /> Actions
              </span>
            </div>
          </div>
        )}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-slate-50 dark:bg-white/5 border-b border-slate-200/60 dark:border-white/10 text-slate-500 dark:text-slate-400">
              <tr>
                {col("organization") && <th className="px-6 py-4 font-semibold text-[13px] uppercase tracking-wider">Organization</th>}
                {col("plan_mrr")     && <th className="px-6 py-4 font-semibold text-[13px] uppercase tracking-wider">Plan & MRR</th>}
                {col("health")       && <th className="px-6 py-4 font-semibold text-[13px] uppercase tracking-wider">Health</th>}
                {col("usage")        && <th className="px-6 py-4 font-semibold text-[13px] uppercase tracking-wider">Usage & Storage</th>}
                {col("security")     && <th className="px-6 py-4 font-semibold text-[13px] uppercase tracking-wider">Security</th>}
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-white/5">
              {loading ? (
                Array.from({ length: 5 }).map((_, i) => (
                  <tr key={i}>
                    {col("organization") && <td className="px-6 py-4"><div className="h-10 bg-slate-100 dark:bg-white/10 rounded-md animate-pulse w-48" /></td>}
                    {col("plan_mrr")     && <td className="px-6 py-4"><div className="h-10 bg-slate-100 dark:bg-white/10 rounded-md animate-pulse w-24" /></td>}
                    {col("health")       && <td className="px-6 py-4"><div className="h-8  bg-slate-100 dark:bg-white/10 rounded-md animate-pulse w-24" /></td>}
                    {col("usage")        && <td className="px-6 py-4"><div className="h-10 bg-slate-100 dark:bg-white/10 rounded-md animate-pulse w-32" /></td>}
                    {col("security")     && <td className="px-6 py-4"><div className="h-8  bg-slate-100 dark:bg-white/10 rounded-md animate-pulse w-16" /></td>}
                    <td className="px-6 py-4"><div className="h-8 bg-slate-100 dark:bg-white/10 rounded-md animate-pulse w-8 ml-auto" /></td>
                  </tr>
                ))
              ) : filteredTenants.length === 0 ? (
                <tr>
                  <td colSpan={visibleColumns.size + 1} className="px-6 py-12 text-center text-slate-500 dark:text-slate-400">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <div className={`w-2 h-2 rounded-full ${
                            tenant.health_score === "Excellent" || tenant.health_score === "Good"
                              ? "bg-emerald-500"
                              : tenant.health_score === "Needs Attention"
                              ? "bg-amber-500"
                              : "bg-rose-500"
                          }`} />
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-1.5">
                          <div className="flex items-center gap-3 text-xs text-slate-600 dark:text-slate-400">
                            <span className="flex items-center gap-1" title="Active Users">
                              <Users className="w-3 h-3" /> {tenant.active_users}/{tenant.users_count}
                            </span>
                            <span className="flex items-center gap-1" title="Storage Used">
                              <HardDrive className="w-3 h-3" /> {tenant.storage_used} GB
                            </span>
                          </div>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button
                          ...
                          className="h-8 w-8 text-slate-400 hover:text-emerald-500 dark:hover:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 rounded-md"
                          title="Impersonate User"
                        >
                          <Shield className="h-4 w-4" />
                        </Button>
                        <Button
                          onClick={e => handleToggleStatus(tenant.id, tenant.is_active, e)}
                          variant="ghost" size="icon"
                          className={`h-8 w-8 text-slate-400 rounded-md ${tenant.is_active ? "hover:text-rose-600" : "hover:text-emerald-500"}`}
                          title={tenant.is_active ? "Suspend Organization" : "Activate Organization"}
                        >
