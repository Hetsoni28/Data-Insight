...
function DraggableWidgetTemplate({ widget }: { widget: any }) {
  ...
  return (
    <div
      ...
      {...listeners}
      {...attributes}
      className={`flex flex-col p-4 mb-3 border rounded-xl bg-white dark:bg-white/5 cursor-grab active:cursor-grabbing hover:border-emerald-500 hover:shadow-lg transition-all duration-300 group ${
        isDragging ? 'opacity-50 border-emerald-500 shadow-xl scale-105' : 'border-slate-200 dark:border-white/10 hover:-translate-y-1'
      }`}
    >
      <div className="flex items-center space-x-3">
        <div className="p-2 bg-slate-50 dark:bg-white/5 rounded-lg text-slate-500 group-hover:text-emerald-500 group-hover:bg-emerald-50 dark:group-hover:bg-emerald-500/10 transition-colors">
          <widget.icon className="w-5 h-5" />
        </div>
        <div className="flex-1">
          <h4 className="text-sm font-semibold text-slate-900 dark:text-white">{widget.title}</h4>
          <p className="text-xs text-slate-500 dark:text-slate-400 font-medium mt-0.5">{widget.description}</p>
        </div>
