  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-card rounded-2xl shadow-xl w-full max-w-lg overflow-hidden border border-slate-200 dark:border-white/10 animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2">
            {datasets.filter(d => d.status === 'ready').length === 0 ? (
              ...
              datasets.filter(d => d.status === 'ready').map(d => (
                ...
                >
                  <div className={`p-2 rounded-lg ${selectedId === d.id ? 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400' : 'bg-slate-100 dark:bg-white/5 text-slate-500'}`}>
        ...
        <div className="p-6 border-t border-slate-200 dark:border-white/10 flex justify-end gap-3 bg-slate-50 dark:bg-white/5">
          <button 
            onClick={onClose}
