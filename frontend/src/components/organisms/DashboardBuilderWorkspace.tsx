...
export function DashboardBuilderWorkspace({ basePath }: { basePath: string }) {
  ...
  const handlePublish = async () => {
    try {
      if (!dashboard?.is_published) {
        // Validation: Ensure all widgets (except markdown/ai) have a dataset attached
        const unconfiguredWidgets = layout.widgets.filter(w => 
          w.type !== 'markdown' && w.type !== 'ai_insight' && !w.config?.dataset_id
        )
        if (unconfiguredWidgets.length > 0) {
          toast.error(`Cannot publish: ${unconfiguredWidgets.length} widget(s) missing data source.`)
          return
        }
      }
  ...
  const handleDuplicate = async () => {
    try {
      ...
      toast.error("Failed to duplicate dashboard", { id: 'duplicate-toast' })
    }
  }
      ...
      <div className="h-14 border-b border-slate-200 dark:border-white/10 bg-white dark:bg-[#0B0F17] flex items-center justify-between px-4 shrink-0">
        ...
        <div className="flex items-center space-x-2">
          ...
          <button 
            ...
            className="flex items-center px-3 py-1.5 text-sm font-medium rounded-lg text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
          >
            Duplicate
          </button>
