  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }}
        className="relative bg-slate-900 border border-white/10 rounded-2xl p-6 w-full max-w-md shadow-2xl z-10">
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-blue-500/20 flex items-center justify-center">
              <Key className="w-4 h-4 text-blue-400" />
            </div>
            <h2 className="text-lg font-bold text-white">Client Credentials</h2>
          </div>
        ...
        <div className="space-y-4">
          <div className="p-4 rounded-xl bg-blue-500/10 border border-blue-500/20">
            <p className="text-xs text-slate-400 mb-1">Application</p>
            <p className="font-semibold text-white">{app.name}</p>
          </div>

          <div>
            <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider block mb-2">Client ID</label>
            <div className="flex items-center gap-2 bg-black/30 rounded-lg px-3 py-2.5 font-mono text-xs text-slate-300">
          ...
          <div>
            <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider block mb-2">Client Secret</label>
            <div className="flex items-center gap-2 bg-black/30 rounded-lg px-3 py-2.5 font-mono text-xs">
              <span className="flex-1 text-emerald-300 break-all">{show ? fakeSecret : fakeSecret.replace(/./g, "•")}</span>
              <button onClick={() => setShow(!show)} className="text-slate-500 hover:text-white shrink-0 transition-colors">
                {show ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
              </button>
              <button onClick={handleCopy} className="text-slate-500 hover:text-emerald-400 shrink-0 transition-colors">
          ...
          <Button onClick={onClose} className="w-full bg-white/10 hover:bg-white/20 text-white border border-white/10 mt-2">Close</Button>
        </div>
      </motion.div>
    </div>
  )
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }}
        ...
        <div className="space-y-5">
          ...
          <div className="flex items-center justify-between p-4 rounded-xl bg-white/5 border border-white/10">
            <div>
              <p className="text-sm font-medium text-white">Application Status</p>
              <p className="text-xs text-slate-400 mt-0.5">Enable or disable this OAuth client</p>
            </div>
            <button
              onClick={() => setActive(!active)}
              className={`relative w-12 h-6 rounded-full transition-colors ${active ? "bg-emerald-500" : "bg-slate-700"}`}>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {app.scopes.map((s: string) => (
                <span key={s} className="px-2.5 py-1 rounded-lg text-xs bg-emerald-500/10 text-emerald-300 border border-emerald-500/20 font-mono">{s}</span>
              ))}
            </div>
          </div>

          <div className="flex gap-3 pt-2">
            <Button variant="outline" onClick={onClose} className="flex-1 bg-white/5 border-white/10 text-white hover:bg-white/10">Cancel</Button>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
        className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={onClose} />
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }}
        className="relative bg-slate-900 border border-red-500/20 rounded-2xl p-6 w-full max-w-md shadow-2xl z-10">
        <div className="text-center mb-5">
          <div className="w-14 h-14 rounded-2xl bg-red-500/20 flex items-center justify-center mx-auto mb-4">
            <Trash2 className="w-6 h-6 text-red-400" />
          </div>
          <h2 className="text-lg font-bold text-white">Revoke Application</h2>
          <p className="text-sm text-slate-400 mt-2">This will immediately revoke all access for <span className="text-white font-medium">"{app.name}"</span>. This action cannot be undone.</p>
        </div>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }}
            className="relative bg-slate-900 border border-white/10 rounded-2xl p-6 w-full max-w-lg shadow-2xl z-10">
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider block mb-2">Application Name <span className="text-red-400">*</span></label>
                <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Mobile Dashboard, Zapier Integration..."
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/50" />
              </div>
              <div>
                <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider block mb-2">Redirect URI <span className="text-red-400">*</span></label>
              ...
              <div className="flex gap-3 pt-2">
                <Button variant="outline" onClick={onClose} className="flex-1 bg-white/5 border-white/10 text-white hover:bg-white/10">Cancel</Button>
                <Button onClick={handleRegister} disabled={registering || done} className="flex-1 bg-blue-600 hover:bg-blue-500 text-white">
  ...
  if (!localApps || localApps.length === 0) {
    return (
      <motion.div
        ...
        className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-3xl overflow-hidden shadow-sm mt-6"
      >
        ...
        <div className="p-6 border-b border-slate-200 dark:border-white/10 flex justify-between items-center">
          ...
          <Button onClick={() => setRegisterOpen(true)} size="sm" className="bg-blue-600 hover:bg-blue-500 text-white">
    ...
    <>
      ...
      >
        <div className="p-6 border-b border-slate-200 dark:border-white/10 flex justify-between items-center bg-slate-50/50 dark:bg-white/[0.02]">
          ...
          <Button onClick={() => setRegisterOpen(true)} size="sm" className="bg-blue-600 hover:bg-blue-500 text-white cursor-pointer">
            <Plus className="w-4 h-4 mr-2" />Register Application
          </Button>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-500 dark:text-slate-400">
            <thead className="bg-slate-50/80 dark:bg-slate-800/50 text-xs uppercase text-slate-500 dark:text-slate-400 font-semibold border-b border-slate-200 dark:border-white/10">
              <tr>
              ...
              {localApps.map((app) => (
                <tr key={app.id} className="hover:bg-slate-50/50 dark:hover:bg-white/[0.02] transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center border border-slate-200 dark:border-white/5">
                  ...
                  <td className="px-6 py-4">
                    {app.is_active ? (
                      ...
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-400 border border-slate-200 dark:border-white/10">
                        Inactive
                      </span>
                    )}
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-wrap gap-1">
                      {app.scopes.slice(0, 2).map((scope: string) => (
                        <span key={scope} className="px-2 py-0.5 rounded text-xs bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-white/10 font-mono">
                          {scope}
                        </span>
                      ))}
                      {app.scopes.length > 2 && (
                        <span className="px-2 py-0.5 rounded text-xs bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-white/10">
                          +{app.scopes.length - 2}
                        </span>
                      )}
                    </div>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <Button
                        ...
                        className="h-8 w-8 text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-500/10 cursor-pointer"
                      >
