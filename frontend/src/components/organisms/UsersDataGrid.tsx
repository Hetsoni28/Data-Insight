...
import {
  User, Building2, MoreHorizontal, CheckCircle2, XCircle, Search, Filter,
  Settings2, Power, Eye, Shield, Key, Clock, Check
} from "lucide-react"
...
type UserColId = typeof USER_COLUMNS[number]["id"]
...
export function UsersDataGrid() {
  ...
  const [visibleColumns, setVisibleColumns] = useState<Set<UserColId>>(
    new Set(USER_COLUMNS.map(c => c.id))
  )

  const toggleColumn = (id: UserColId) => {
  ...
  const col = (id: UserColId) => visibleColumns.has(id)
  ...
  const impersonateMutation = useMutation({
    ...
    onSuccess: () => toast.success("Impersonation mode initiated. Redirecting..."),
  ...
  const resetPasswordMutation = useMutation({
    ...
    onSuccess: (_data, email) => toast.success(`Password reset email sent to ${email}.`),
  ...
  const handleViewDetails = (user: any) => { setSelectedUser(user); setIsSheetOpen(true) }

  const handleImpersonate = (user: any) => {
    if (!user.tenant_id || !user.id) { toast.error("User or Tenant ID missing."); return }
    const reason = window.prompt(
      `Please provide a reason for impersonating ${user.email} (e.g. ticket #, debugging, customer request):`
    )
    if (!reason?.trim()) { toast.info("Impersonation cancelled (reason is required for audit logs)."); return }
        ...
        <div className="p-4 md:p-5 border-b border-slate-100 dark:border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50/50 dark:bg-white/[0.02]">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              className={`h-10 bg-white dark:bg-white/5 border-slate-200/60 dark:border-white/10 shadow-sm rounded-md hidden md:flex ${!isColumnPickerOpen ? "text-slate-700 dark:text-slate-300" : ""}`}
            >
              <Settings2 className="h-4 w-4 mr-2" />
              Columns
              {visibleColumns.size < USER_COLUMNS.length && (
        ...
        {isColumnPickerOpen && (
          <div className="p-4 border-b border-slate-200/60 dark:border-white/10 bg-slate-50/50 dark:bg-white/[0.02]">
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                onClick={() => setVisibleColumns(new Set(USER_COLUMNS.map(c => c.id)))}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-slate-50 dark:bg-white/[0.03] border-b border-slate-200/60 dark:border-white/10 text-slate-500 dark:text-slate-400">
              <tr>
                {col("user")         && <th className="px-6 py-4 font-semibold text-[13px] uppercase tracking-wider">User</th>}
                {col("organization") && <th className="px-6 py-4 font-semibold text-[13px] uppercase tracking-wider">Organization</th>}
                {col("role")         && <th className="px-6 py-4 font-semibold text-[13px] uppercase tracking-wider">Role</th>}
                {col("status")       && <th className="px-6 py-4 font-semibold text-[13px] uppercase tracking-wider">Status</th>}
                {col("joined")       && <th className="px-6 py-4 font-semibold text-[13px] uppercase tracking-wider">Joined</th>}
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-white/10">
              {loading ? (
                Array.from({ length: 5 }).map((_, i) => (
                  <tr key={i}>
                    {col("user")         && <td className="px-6 py-4"><div className="h-10 bg-slate-100 dark:bg-white/10 rounded-md animate-pulse w-48" /></td>}
                    {col("organization") && <td className="px-6 py-4"><div className="h-6  bg-slate-100 dark:bg-white/10 rounded-md animate-pulse w-32" /></td>}
                    {col("role")         && <td className="px-6 py-4"><div className="h-6  bg-slate-100 dark:bg-white/10 rounded-md animate-pulse w-20" /></td>}
                    {col("status")       && <td className="px-6 py-4"><div className="h-6  bg-slate-100 dark:bg-white/10 rounded-md animate-pulse w-20" /></td>}
                    {col("joined")       && <td className="px-6 py-4"><div className="h-6  bg-slate-100 dark:bg-white/10 rounded-md animate-pulse w-24" /></td>}
              ...
              <div className="flex-1 space-y-6 overflow-y-auto pr-2">
                ...
                <div>
                  ...
                  <Button
                    ...
                    className={`w-full justify-start h-10 ${selectedUser.is_active ? "text-rose-600 hover:text-rose-700 hover:bg-rose-50 dark:text-rose-400 dark:hover:bg-rose-500/10" : "text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50 dark:text-emerald-400 dark:hover:bg-emerald-500/10"}`}
                  >
