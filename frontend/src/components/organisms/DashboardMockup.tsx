...
export function DashboardMockup() {
  ...
  return (
    <div className="w-full rounded-2xl border border-slate-200 shadow-2xl overflow-hidden bg-slate-50 relative z-10">
      {/* Browser Bar */}
      <div className="flex items-center gap-1.5 px-4 py-2.5 bg-slate-100 border-b">
          ...
          {TABS.map(({ icon: Icon, label, id }) => {
            ...
            return (
              <button
                key={id}
                onClick={() => setActiveTab(id)}
                className={`flex items-center gap-2 px-2.5 py-2 rounded-lg text-[11px] font-medium transition-all text-left ${active ? "bg-[#10B981] text-white shadow-md scale-[1.02]" : "text-slate-500 hover:bg-slate-50"}`}
              >
          ...
          <AnimatePresence mode="wait">
            {activeTab === "overview" && (
              <motion.div key="overview" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4 h-full flex flex-col">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-semibold text-slate-800">Revenue Dashboard</span>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[{ label: "Revenue", value: "$2.4M", change: "+18.4%" }, { label: "Clients", value: "142", change: "+12%" }, { label: "Churn", value: "1.2%", change: "-0.3%" }].map(kpi => (
                    <div key={kpi.label} className="bg-white rounded-xl border border-slate-200 shadow-sm p-3 space-y-1 group hover:border-[#10B981]/40 transition-colors cursor-default">
                ...
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-3 flex-1 flex flex-col relative overflow-hidden group">
                  <div className="text-[11px] font-semibold text-slate-700 mb-2">Monthly Revenue & Forecast</div>
            ...
            {activeTab === "datasets" && (
              <motion.div key="datasets" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4 h-full flex flex-col">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-semibold text-slate-800">Connected Data Sources</span>
                ...
                <div className="space-y-2.5 overflow-y-auto pr-1 flex-1">
                  {[
                    ...
                    return (
                      <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} className="bg-white rounded-xl border border-slate-200 p-3 flex items-center justify-between shadow-sm">
            ...
            {activeTab === "reports" && (
              <motion.div key="reports" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4 h-full flex flex-col">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-semibold text-slate-800">AI Generated Reports</span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 overflow-y-auto pr-1">
                  {reports.map((rep, i) => (
                    <motion.div key={rep.title} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.05 }} className="bg-white rounded-xl border border-slate-200 p-3.5 flex flex-col justify-between shadow-sm min-h-[90px] group hover:border-[#10B981]/50 cursor-pointer">
                      <div className="space-y-1.5">
                        <div className="flex items-start gap-2">
                          <FileText className="h-3.5 w-3.5 text-slate-400 mt-0.5 group-hover:text-[#10B981] transition-colors" />
                          <div>
                            <div className="text-xs font-semibold text-slate-800 line-clamp-2 leading-tight group-hover:text-[#10B981] transition-colors">{rep.title}</div>
                  ...
                  {reports.length === 0 && (
                    <div className="col-span-2 py-8 text-center text-xs text-slate-500">No reports found.</div>
                  )}
                </div>
              </motion.div>
            )}
            ...
            {activeTab === "ai" && (
              <motion.div key="ai" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="h-full flex flex-col">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-sm font-semibold text-slate-800">Data Insight Copilot</span>
                </div>
                ...
                <div className="flex-1 bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
                  <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    {messages.map((msg, i) => (
                      <motion.div key={i} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className={`flex gap-3 max-w-[90%] ${msg.role === 'user' ? 'ml-auto flex-row-reverse' : ''}`}>
                        ...
                        <div className={`rounded-xl p-2.5 text-[11px] leading-relaxed shadow-sm ${msg.role === 'user' ? 'bg-slate-800 text-white rounded-tr-none' : 'bg-[#10B981]/10 text-slate-800 rounded-tl-none border border-[#10B981]/20'}`}>
                          {msg.text}
                          {msg.hasAction && (
                            <button className="bg-white border border-[#10B981]/30 text-[#10B981] px-2.5 py-1.5 rounded-lg shadow-sm text-[9px] font-bold flex items-center gap-1.5 mt-2 hover:bg-[#10B981]/5 transition-colors">
                  ...
                  <form onSubmit={handleSendChat} className="p-2 border-t bg-slate-50 flex items-center gap-2">
                    <Input 
                      value={chatInput} onChange={e => setChatInput(e.target.value)}
                      placeholder="Ask any data question..." 
                      className="h-8 text-xs bg-white border-slate-200 focus-visible:ring-[#10B981]" 
                    />
            ...
            {activeTab === "forecast" && (
              <motion.div key="forecast" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4 h-full flex flex-col">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-semibold text-slate-800">Q4 Revenue Forecast</span>
                  <Badge variant="outline" className="text-[10px] text-[#10B981] border-[#10B981] bg-[#10B981]/5 px-2">94% Confidence</Badge>
                </div>
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 flex-1 flex flex-col justify-between">
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="text-[11px] font-medium text-slate-500 mb-0.5">Predicted Q4 ARR</div>
                      <div className="text-2xl font-black text-slate-900">$3.1M</div>
                    </div>
                    <div className="text-right">
                      <div className="text-[11px] font-medium text-slate-500 mb-0.5">Target: $3.0M</div>
