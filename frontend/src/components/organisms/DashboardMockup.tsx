...
export function DashboardMockup() {
  ...
  return (
    <div className="w-full rounded-2xl border border-slate-200 dark:border-white/10 shadow-2xl overflow-hidden bg-slate-50 dark:bg-white/5 relative z-10">
      {/* Browser Bar */}
      <div className="flex items-center gap-1.5 px-4 py-2.5 bg-slate-100 dark:bg-white/10 border-b">
        <div className="h-2.5 w-2.5 rounded-full bg-red-400" />
        <div className="h-2.5 w-2.5 rounded-full bg-yellow-400" />
        <div className="h-2.5 w-2.5 rounded-full bg-green-400" />
        <div className="ml-3 flex-1 bg-white dark:bg-white/5 rounded border text-[10px] text-slate-400 px-3 py-0.5 max-w-[180px] shadow-sm">app.datainsight.ai</div>
      </div>

      <div className="flex h-[360px] sm:h-[400px] flex-col sm:flex-row">
        {/* Mobile tab bar — only visible on small screens */}
        <div className="flex sm:hidden overflow-x-auto border-b bg-white dark:bg-white/5 px-1 shrink-0">
        ...
        {/* Desktop Sidebar */}
        <div className="hidden sm:flex w-36 border-r bg-white dark:bg-white/5 flex-col gap-1 p-3 shrink-0">
          {TABS.map(({ icon: Icon, label, id }) => {
            const active = activeTab === id
            return (
              <button
          ...
          <AnimatePresence mode="wait">
              ...
              <motion.div key="overview" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4 h-full flex flex-col">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[{ label: "Revenue", value: "$2.4M", change: "+18.4%" }, { label: "Clients", value: "142", change: "+12%" }, { label: "Churn", value: "1.2%", change: "-0.3%" }].map(kpi => (
                    <div key={kpi.label} className="bg-white dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/10 shadow-sm p-3 space-y-1 group hover:border-[#10B981]/40 transition-colors cursor-default">
                ...
                <div className="bg-white dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/10 shadow-sm p-3 flex-1 flex flex-col relative overflow-hidden group">
              ...
              <motion.div key="datasets" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4 h-full flex flex-col">
                ...
                <div className="space-y-2.5 overflow-y-auto pr-1 flex-1">
                  {[
                    ...
                    return (
                      <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} className="bg-white dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/10 p-3 flex items-center justify-between shadow-sm">
              ...
              <motion.div key="reports" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4 h-full flex flex-col">
                ...
                <div className="relative">
                  ...
                  <Input 
                    ...
                    className="h-8 pl-8 text-xs bg-white dark:bg-white/5 border-slate-200 dark:border-white/10"
                    value={reportSearch}
                ...
                <div className="grid grid-cols-2 gap-3 overflow-y-auto pr-1">
                  {reports.map((rep, i) => (
                    <motion.div key={rep.title} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.05 }} className="bg-white dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/10 p-3.5 flex flex-col justify-between shadow-sm min-h-[90px] group hover:border-[#10B981]/50 cursor-pointer">
            ...
            {activeTab === "ai" && (
              <motion.div key="ai" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="h-full flex flex-col">
                ...
                <div className="flex-1 bg-white dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/10 shadow-sm overflow-hidden flex flex-col">
                    ...
                    {messages.map((msg, i) => (
                      <motion.div key={i} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className={`flex gap-3 max-w-[90%] ${msg.role === 'user' ? 'ml-auto flex-row-reverse' : ''}`}>
                        ...
                        <div className={`rounded-xl p-2.5 text-[11px] leading-relaxed shadow-sm ${msg.role === 'user' ? 'bg-slate-800 text-white rounded-tr-none' : 'bg-[#10B981]/10 text-slate-800 dark:text-slate-200 rounded-tl-none border border-[#10B981]/20'}`}>
                          {msg.text}
                          {msg.hasAction && (
                            <button className="bg-white dark:bg-white/5 border border-[#10B981]/30 text-[#10B981] px-2.5 py-1.5 rounded-lg shadow-sm text-[9px] font-bold flex items-center gap-1.5 mt-2 hover:bg-[#10B981]/5 transition-colors">
                  ...
                  <form onSubmit={handleSendChat} className="p-2 border-t bg-slate-50 dark:bg-white/5 flex items-center gap-2">
                    <Input 
                      value={chatInput} onChange={e => setChatInput(e.target.value)}
                      placeholder="Ask any data question..." 
                      className="h-8 text-xs bg-white dark:bg-white/5 border-slate-200 dark:border-white/10 focus-visible:ring-[#10B981]" 
                    />
            ...
            {activeTab === "forecast" && (
              <motion.div key="forecast" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4 h-full flex flex-col">
                ...
                <div className="bg-white dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/10 shadow-sm p-4 flex-1 flex flex-col justify-between">
