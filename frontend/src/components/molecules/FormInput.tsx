...
export const FormInput = forwardRef<HTMLInputElement | HTMLTextAreaElement, FormInputProps>(
  ({ label, description, error, className, multiline, ...props }, ref) => {
    const inputClasses = cn(
      "w-full px-4 py-2.5 bg-slate-50 border border-slate-200/60 rounded-xl text-sm text-slate-900 transition-all duration-200",
      "focus:outline-none focus:bg-white focus:border-emerald-500/50 focus:ring-4 focus:ring-emerald-500/10",
    ...
    return (
      <div className="space-y-1.5 w-full">
        ...
        {description && (
          <p className="text-[12px] text-slate-500 mb-2">{description}</p>
        )}
